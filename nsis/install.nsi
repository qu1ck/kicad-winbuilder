...
SectionGroup /e $(TITLE_SEC_LIBRARIES) SEC03
  ...
  Section /o $(TITLE_SEC_PACKAGES3D) SEC03_PACKAGES3D
    ...
    IfFileExists "$EXEDIR\${KICAD_PACKAGES3D_FILE}" unzip download

    download:
    DetailPrint "Downloading ${KICAD_PACKAGES3D_URL}"
    inetc::get /caption "Footprint 3D models" /popup "" ${KICAD_PACKAGES3D_URL} "$EXEDIR\${KICAD_PACKAGES3D_FILE}" /end
    Pop $R0
    ...
    complete:
    DetailPrint "Downloaded footprint 3d models archive to $EXEDIR\${KICAD_PACKAGES3D_FILE}"

    unzip:
    nsisunz::UnzipToLog "$EXEDIR\${KICAD_PACKAGES3D_FILE}" "$INSTDIR\share\kicad\modules\"
    Pop $R0
    ${If} $R0 == "success"
      IfFileExists "$INSTDIR\share\kicad\modules\packages3d" +1 +2
        RMDir /r "$INSTDIR\share\kicad\modules\packages3d"
      Rename "$INSTDIR\share\kicad\modules\${KICAD_PACKAGES3D_FOLDER}" "$INSTDIR\share\kicad\modules\packages3d"
      MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON1 $(CLEANUP_PROMPT) /SD IDYES IDNO +2
        Delete "$EXEDIR\${KICAD_PACKAGES3D_FILE}"
    ${Else}
      DetailPrint "Extracting 3d models failed: $R0"
      MessageBox MB_ICONEXCLAMATION|MB_OK "Extracting 3d models failed: $R0"
      Abort "Installation aborted."
    ${EndIf}
  SectionEnd
  !endif
